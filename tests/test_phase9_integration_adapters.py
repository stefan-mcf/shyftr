"""Phase 9 generic evidence, closeout adapter, and retrieval usage contract tests."""
...
def test_generic_evidence_adapter_ingests_idempotently_and_extracts(tmp_path: Path) -> None:
    cell = init_cell(tmp_path, "phase9-cell", cell_type="user")
    ...
    assert source_rows[0]["metadata"]["adapter_id"] == "phase9-generic"
...
def test_generic_evidence_adapter_boundary_failure_does_not_append(tmp_path: Path) -> None:
    cell = init_cell(tmp_path, "phase9-boundary", cell_type="user")
...
def test_closeout_artifact_adapter_preserves_artifact_and_ingests(tmp_path: Path) -> None:
    cell = init_cell(tmp_path, "phase9-artifact", cell_type="user")
    ...
    adapter = CloseoutArtifactAdapter(closeouts, external_system="external-runtime", external_scope="phase9")
    result = ingest_sources_from_adapter(cell, adapter)
...
def test_retrieval_usage_logs_are_filtered_and_sanitized(tmp_path: Path) -> None:
    cell = init_cell(tmp_path, "phase9-logs", cell_type="user")
    append_jsonl(
...
def test_phase9_built_in_adapters_are_listed() -> None:
