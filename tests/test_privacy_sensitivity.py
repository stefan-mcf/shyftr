...
def _append_charge(cell: Path, trace_id: str, *, sensitivity: str = "internal", user_id: str = "stefan", project_id: str = "shyftr") -> dict:
    row = {
        "trace_id": trace_id,
...
def test_privacy_policy_excludes_sensitive_or_cross_scope_memory_by_default(tmp_path: Path) -> None:
    ...
    policy = AccessPolicy(runtime_id="runtime-a", user_id="stefan", project_id="shyftr")
...
def test_redaction_projection_hides_sensitive_text_without_destroying_provenance(tmp_path: Path) -> None:
    ...
    audit_policy = AccessPolicy(runtime_id="runtime-a", user_id="stefan", project_id="shyftr", allowed_sensitivity=("public", "internal"), allow_audit_sensitive=True)
