...
def test_unapproved_fragment_cannot_be_promoted(tmp_path):
    ...
    with pytest.raises(PromotionError, match="approved"):
        promote_fragment(cell_path, fragment.fragment_id, promoter="stefan")
...
def test_latest_review_must_be_approved_to_promote(tmp_path):
    cell_path = init_cell(tmp_path, "core")
    _, fragment = _fragment(cell_path, tmp_path / "source.md")
    approve_fragment(cell_path, fragment.fragment_id, reviewer="stefan", rationale="useful")
    reject_fragment(cell_path, fragment.fragment_id, reviewer="stefan", rationale="superseded")
...
def test_promote_approved_fragment_appends_trace_and_promotion_event(tmp_path):
    ...
    approve_fragment(cell_path, fragment.fragment_id, reviewer="stefan", rationale="durable and bounded")

    trace = promote_fragment(cell_path, fragment.fragment_id, promoter="stefan")
...
def test_promote_is_idempotent_for_same_fragment(tmp_path):
    cell_path = init_cell(tmp_path, "core")
    _, fragment = _fragment(cell_path, tmp_path / "source.md")
    approve_fragment(cell_path, fragment.fragment_id, reviewer="stefan", rationale="durable")

    first = promote_fragment(cell_path, fragment.fragment_id, promoter="stefan")
    second = promote_fragment(cell_path, fragment.fragment_id, promoter="stefan")
