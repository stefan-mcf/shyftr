...
def test_accepting_missing_memory_promotion_is_explicitly_unimplemented(tmp_path: Path) -> None:
    cell = init_cell(tmp_path, "evo-cell")
    proposal = EvolutionProposal(
        ...
        confidence=0.6,
        risk_level="medium",
        proposed_memory={"statement": "Synthetic missing memory placeholder."},
    ).to_dict()
    append_evolution_proposal(cell, proposal)
    with pytest.raises(NotImplementedError, match="promote_missing_memory"):
        review_evolution_proposal(cell, proposal["proposal_id"], decision="accept", rationale="operator checked placeholder", actor="test")
    assert list(read_jsonl(cell / "ledger" / "evolution" / "reviews.jsonl")) == []
