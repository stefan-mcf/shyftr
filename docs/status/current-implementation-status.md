...
Status: local-first alpha / controlled-pilot release.
...
| Trusted memory provider integration | partial | qualified | provider modules and tests only | no hosted provider control plane | `src/shyftr/provider/memory.py`, `src/shyftr/provider/trusted.py` | `tests/test_trusted_memory.py`, `tests/test_memory_provider.py` | Controlled-pilot integration surface; do not imply managed backend replacement for all domains. |
| pattern/rule distillation | partial/planned | qualified/no current broad claim | older compatibility modules/tests | no public UI claim | `src/shyftr/distill/patterns.py`, `src/shyftr/distill/rules.py` | `tests/test_patterns.py`, `tests/test_rules.py` | Treat as future-facing unless directly citing tested local behavior. |
