...
| candidate extraction/review | implemented | yes, with legacy alias note | `shyftr candidate`, `shyftr approve`, `shyftr reject` | console candidate queue and review endpoint | `src/shyftr/extract.py`, `src/shyftr/review.py` | `tests/test_extract.py`, `tests/test_review.py`, `tests/test_console_api.py` | Legacy candidate terms remain in compatibility APIs. |
| memory promotion | implemented | yes | `shyftr memory` / `shyftr promote` | memory explorer and lifecycle action endpoints | `src/shyftr/promote.py`, `src/shyftr/mutations.py` | `tests/test_promote.py`, `tests/test_memory_mutations.py` | Compatibility fields still use memory identifiers in places. |
...
| pack generation | implemented | yes | `shyftr pack` / `shyftr pack` | `POST /pack`, cell pack endpoint | `src/shyftr/pack.py`, `src/shyftr/integrations/pack_api.py` | `tests/test_pack.py`, `tests/test_pack_api.py` | Legacy pack naming remains as CLI alias and data compatibility. |
| feedback recording | implemented | yes | `shyftr feedback` / `shyftr feedback` | `POST /feedback`, cell feedback endpoint | `src/shyftr/feedbacks.py`, `src/shyftr/integrations/feedback_api.py` | `tests/test_feedbacks.py`, `tests/test_feedback_api.py` | Legacy feedback naming remains as CLI alias and data compatibility. |
...
| pattern/rule distillation | partial/planned | qualified/no current broad claim | older compatibility modules/tests | no public UI claim | `src/shyftr/patterns.py`, rule docs | `tests/test_patterns.py`, `tests/test_rule.py` | Treat as future-facing unless directly citing tested local behavior. |
