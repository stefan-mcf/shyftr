...
- The power vocabulary and lifecycle: Pulse -> Spark -> Charge -> Coil -> Rail.
...
- The safety posture: append-only ledgers, review gates, provenance, explainable Pack selection, rollback/fallback during pilots, and measured Signals.
- The product promise: capture evidence, promote reviewed memory, retrieve bounded Packs, record Signals, learn from outcomes, and maintain auditability.
...
3. Replace the first-party Antaeus proving ground with one real, repeatable, runtime-neutral proof loop.
   - Candidate A: generic local assistant/runtime harness that requests Packs before tasks and records Signals after tasks.
   - Candidate B: Hermes-style adapter if Hermes remains active as a generic runtime target, not as Antaeus.
   - Candidate C: markdown/docs workspace adapter that ingests notes/docs as Pulses, proposes Sparks, and drives review.

4. Move toward contract completion before more application surface.
   - Provider contract front door: remember, search, profile, forget/replace/deprecate, pack, record_signal, import/export.
   - Closed-loop priority: Pack + Signal + retrieval logs + confidence/provenance explanation.
...
3. How much of Phase 2 UI is required before authority pilots?
   - A Pack debugger, Signal viewer, and review queue may be safety requirements rather than polish.
