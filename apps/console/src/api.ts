...
export const api = {
  ...
  memories: (root: string, cellId: string, query = "") => request<RowList<Json>>(`/cell/${encodeURIComponent(cellId)}/memories?${rootParam(root)}${query ? `&query=${encodeURIComponent(query)}` : ""}`),
  ...
  evolution: (cellPath: string) => request<RowList<Json>>(`/evolution?cell_path=${encodeURIComponent(cellPath)}`),
  evolutionScan: (cellPath: string, writeProposals = false) => request<Json>("/evolution/scan", { method: "POST", body: JSON.stringify({ cell_path: cellPath, write_proposals: writeProposals }) }),
  evolutionSimulate: (cellPath: string, proposalId: string, appendReport = false) => request<Json>(`/evolution/${encodeURIComponent(proposalId)}/simulate`, { method: "POST", body: JSON.stringify({ cell_path: cellPath, append_report: appendReport }) }),
  evolutionReview: (cellPath: string, proposalId: string, body: Json) => request<Json>(`/evolution/${encodeURIComponent(proposalId)}/review`, { method: "POST", body: JSON.stringify({ cell_path: cellPath, ...body }) }),
  ...
  memoryAction: (root: string, cellId: string, memoryId: string, body: Json) => request<Json>(`/cell/${encodeURIComponent(cellId)}/memories/${encodeURIComponent(memoryId)}/action?${rootParam(root)}`, { method: "POST", body: JSON.stringify(body) }),
