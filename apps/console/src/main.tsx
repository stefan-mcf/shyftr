...
const nav = ["Cells", "Review", "Memories", "Packs", "Feedback", "Frontier", "Evolution", "Multi-cell", "Reports", "Settings"] as const;
...
function App() {
  ...
  async function loadTab(target: Tab = tab) {
    ...
    const loaders: Partial<Record<Tab, () => Promise<Json>>> = {
      ...
      Memories: () => api.memories(root, cellId, query),
      Reports: async () => ({
      ...
      Evolution: () => api.evolution(String(selectedCell?.cell_path || summary?.cell_path || "")),
    };
  ...
  return <main>
    <header>
      <div><h1>ShyftR Console</h1><p>Local-first Cell dashboard, review queue, Pack debugger, Feedback console, and pilot metrics.</p></div>
    ...
    {tab === "Cells" && <section>
      ...
      <div className="cards">
        <Card label="Pulses" value={counts.pulses} /><Card label="Sparks" value={counts.sparks} /><Card label="Pending review" value={counts.pending_review} /><Card label="Approved memories" value={counts.approved_memories} />
        ...
        <Card label="Evolution proposals" value={counts.evolution_proposals} /><Card label="Open proposals" value={counts.open_proposals} /><Card label="Hygiene warnings" value={counts.hygiene_warnings} /><Card label="Grid" value={summary?.grid_status || "empty"} />
      </div>
    ...
    {tab === "Memories" && <section><h2>Memory Explorer</h2><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="search memories" /><button onClick={() => loadTab("Memories")}>Search</button><div className="actions"><input value={targetId} onChange={(e) => setTargetId(e.target.value)} placeholder="memory id" /><input value={rationale} onChange={(e) => setRationale(e.target.value)} placeholder="lifecycle reason" /><button onClick={async () => { const r = await run(() => api.memoryAction(root, cellId, targetId, { action: "deprecate", reason: rationale })); if (r) { setActionResult(r); loadTab("Memories"); } }}>Deprecate</button><button onClick={async () => { const r = await run(() => api.memoryAction(root, cellId, targetId, { action: "challenge", reason: rationale })); if (r) { setActionResult(r); loadTab("Memories"); } }}>Challenge</button><button onClick={async () => { const r = await run(() => api.memoryAction(root, cellId, targetId, { action: "forget", reason: rationale })); if (r) { setActionResult(r); loadTab("Memories"); } }}>Forget</button></div><JsonBlock value={actionResult || rows || "Search to inspect provenance, lifecycle state, feedback, and confidence events. Enter a memory id to append a lifecycle event."} /></section>}

    {tab === "Packs" && <section><h2>Pack Debugger</h2><textarea value={query} onChange={(e) => setQuery(e.target.value)} /><label>Retrieval mode <select value={retrievalMode} onChange={(e) => setRetrievalMode(e.target.value)}>{["balanced", "conservative", "exploratory", "risk_averse", "audit", "rule_only", "low_latency"].map((mode) => <option key={mode} value={mode}>{mode}</option>)}</select></label><button onClick={async () => { const p = await run(() => api.pack(root, cellId, { query, external_system: "console", external_scope: "debugger", max_items: 10, max_tokens: 2000, retrieval_mode: retrievalMode })); if (p) setPack(p); }}>Generate Pack</button><JsonBlock value={pack || "Generate a Pack to inspect selected items, score details, risks, retrieval mode, graph context, and token budget."} /></section>}

    {tab === "Feedback" && <section><h2>Feedback Console</h2><textarea value={JSON.stringify({ loadout_id: (pack?.loadout_id as string) || "", result: "success", external_system: "console", external_scope: "manual", applied_memory_ids: pack?.selected_ids || [], useful_memory_ids: pack?.selected_ids || [], missing_memory_notes: [], verification_evidence: {} }, null, 2)} readOnly /><button onClick={async () => { const s = await run(() => api.signal(root, cellId, { loadout_id: (pack?.loadout_id as string) || "manual", result: "success", external_system: "console", external_scope: "manual", applied_memory_ids: pack?.selected_ids || [], useful_memory_ids: pack?.selected_ids || [], missing_memory_notes: [], verification_evidence: { console: true } })); if (s) setSignal(s); }}>Record Feedback</button><JsonBlock value={signal || "Record feedback after a Pack is used."} /></section>}
    ...
    {tab === "Evolution" && <section><h2>Memory evolution</h2><p>Review-gated proposal inbox for split, merge, supersession, replacement, deprecation, forgetting, and redaction. Scans are dry-run by default; no hidden auto-apply exists.</p><div className="actions"><button onClick={() => loadTab("Evolution")}>Load proposals</button><button onClick={async () => { const r = await run(() => api.evolutionScan(String(selectedCell?.cell_path || summary?.cell_path || ""), false)); if (r) setRows(r); }}>Dry-run scan</button><button onClick={async () => { const r = await run(() => api.evolutionScan(String(selectedCell?.cell_path || summary?.cell_path || ""), true)); if (r) { setRows(r); refreshSummary(); } }}>Write proposals only</button><input value={targetId} onChange={(e) => setTargetId(e.target.value)} placeholder="proposal id" /><input value={rationale} onChange={(e) => setRationale(e.target.value)} placeholder="review rationale" /><button onClick={async () => { const r = await run(() => api.evolutionSimulate(String(selectedCell?.cell_path || summary?.cell_path || ""), targetId, true)); if (r) setSimulation(r); }}>Simulate</button><button onClick={async () => { const r = await run(() => api.evolutionReview(String(selectedCell?.cell_path || summary?.cell_path || ""), targetId, { decision: "defer", rationale, simulation_ref: simulation?.simulation_id })); if (r) { setActionResult(r); loadTab("Evolution"); refreshSummary(); } }}>Defer</button><button onClick={async () => { const r = await run(() => api.evolutionReview(String(selectedCell?.cell_path || summary?.cell_path || ""), targetId, { decision: "reject", rationale, simulation_ref: simulation?.simulation_id })); if (r) { setActionResult(r); loadTab("Evolution"); refreshSummary(); } }}>Reject</button><button onClick={async () => { const r = await run(() => api.evolutionReview(String(selectedCell?.cell_path || summary?.cell_path || ""), targetId, { decision: "accept", rationale, simulation_ref: simulation?.simulation_id })); if (r) { setActionResult(r); loadTab("Evolution"); refreshSummary(); } }}>Accept simulated</button></div><JsonBlock value={actionResult || simulation || rows || "Load proposals, run a dry-run scan, simulate a proposal, then record an operator decision."} /></section>}
