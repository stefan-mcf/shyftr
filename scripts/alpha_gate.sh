...
if sys.version_info < (3, 11):
    raise SystemExit(f"ShyftR alpha gate requires Python 3.11+; got {sys.version.split()[0]}")
PY
"$PYTHON_BIN" -m shyftr.cli --help >/dev/null

echo "== Public alpha/readiness posture =="
...
with TemporaryDirectory(prefix="shyftr-alpha-gate-") as tmp:
    cell = init_cell(Path(tmp) / "alpha-gate-cell", "alpha-gate-cell", cell_type="user")
